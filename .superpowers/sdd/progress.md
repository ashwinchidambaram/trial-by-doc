...
NEXT ACTIONS (ordered):
  ...
  4. Part B (reader upgrade): IN FLIGHT — implementer subagent a81bb8c6e38d7e8c3 dispatched (brief scratchpad/partB_brief.md, report scratchpad/partB_report.md). Scope split: SUBAGENT does CODE+FREE-LOCAL only (B0 spec; default_local→Phi-4-mini @cfbefacb99257ffa30c83adab238a50856ac3083 MIT; keep Qwen-1.5B as named ladder rung; OpenRouterReader class via OPEN_ROUTER_API_KEY; register gpt5mini→openai/gpt-5.4-mini + haiku45→anthropic/claude-haiku-4.5 via OpenRouter w/ live-verified slugs+pricing; verify Phi-4-mini loads + Tier-B local smoke; B.1 unchanged; NO paid API calls). CONTROLLER runs the paid ladder afterward (spend control, ~$2 est of $5-10 budget). Phi-4-mini is NATIVELY integrated in transformers (not remote-code) → should avoid the v5 breakage. When subagent reports: verify diff, confirm it's fully dead, then run the API ladder myself.
  ✅ PART B SUBAGENT DONE + INDEPENDENTLY VERIFIED (commits e5eea22 code, e7683ba findings; 74 tests pass; reader identity=Phi-4-mini confirmed in raw; OpenRouterReader code correct incl. import os + os.environ[OPEN_ROUTER_API_KEY]; key in .env + loaded via cli.py:32 load_dotenv; slugs/pricing live-verified). LADDER (paid, controller): fixed input=olmocr2 realdoc OCR (100 samples, rl-* run-ids, v1-baseline untouched). rl-phi4 local FREE DONE B.2=0.18 (floor). rl-gpt5+rl-haiku OpenRouter ~$2. ✅ LADDER COMPLETE (0 errors): B.2 exact/ANLS — Phi-4-mini 0.18/0.692 (floor), gpt-5.4-mini 0.50/0.844 (best), haiku-4.5 0.41/0.761. findings/partb-reader-ladder.md. → PART B DONE (commit fe91373). 
  5. C2 (README auto-inject): IN FLIGHT — subagent a5e69aafd91d80c52 (brief scratchpad/partC2_brief.md, report partC2_report.md). Adds net-new CPU-vs-GPU cost table (A2 throughput × live-verified SKU $/hr) + wires perf/tier-b/cost into inject_readme; regenerates 14-model README from run-id v1-baseline; commits README + finalized v1-baseline scoreboard.csv (currently uncommitted). NO model re-runs, NO paid calls. ✅ C2 DONE + VERIFIED (commit 57ac314, 78 tests). README block regenerated: 14-model scoreboard + Tier-B + perf + NEW CPU-vs-GPU cost table (tesseract CPU $0.057/1k, doctr GPU $0.041/1k cheapest, easyocr CPU $3.95 impractical). SKU pricing live-verified (c6i.xlarge $0.17/hr, g5.xlarge $1.006/hr, Vantage 2026-07-09).
  REMAINING: Part D (scanned robustness — spec docs/superpowers/specs/2026-07-09-scanned-robustness-design.md) + C3 (dashboard UI; C3a read-only autonomously, C3b config/system-prompt = owner-input/later). Plan: delegate Part-D BUILD (degrade.py+realdoc_qa_scanned bench+validate) to subagent, controller runs the scored clean/light/heavy comparison. Then C3a.
  PART D BUILD ✅ (commit 1467b9f, 90 tests; benches realdoc_qa_scanned_light/heavy, validated, degradation seeded/verified). PART D SCORED RUN ✅ COMPLETE (2026-07-09 ~19:22, PARTD_RUN_EXIT=0 / PARTD_ALLDONE): 14 models × {light,heavy} × 100 into v1-baseline, 0 error rows (all 42 model×variant cells 100/100), scoreboard integrated (6719 scored samples). Robustness findings written → findings/partd-scanned-robustness.md. HEADLINE: VLMs scan-robust (olmocr2 75% / gemma4 80% heavy-retained), tesseract/easyocr COLLAPSE (29%/22%) → Part-A "classic engines competitive" holds for CLEAN not scanned; docTR is the robust classic (58%). ⚠️ data-hygiene: clean raw/lightonocr/realdoc_qa.jsonl has 200 rows (recompute 0.464 vs official 0.658) — audit separately, does NOT affect scanned (all clean 100/100).
  NEXT (Part-D wrap): (1) ✅ README reorg + scanned-robustness section DONE (commit 098f200: TOC, scores lead-in, hand-authored clean→light→heavy curve section outside the auto-block). (2) ✅ RENDERER FOLLOW-UP DONE (commit 361b7e8): added _is_derived_bench(), excluded `*_scanned_*` from render() main table + _perf() + footer count (include_derived=True opts in); render_tier_b was already realdoc_qa-only. Fixed cost-caveat above→below. Re-injected: main/tier-b/perf byte-identical to published (footer stays 4199 core, not 6719) — only word changed. New test tests/test_scoreboard_render_derived.py; 103 tests pass. (3) ✅ FINAL WHOLE-BRANCH REVIEW DONE (2 bg reviewers, merge-base 425243f, 26 commits/47 files/+5397).
   CORE (A) = PASS: all 5 binding invariants VERIFIED untouched (frozen scorers empty-diff; instruments still Qwen2.5-7B@a09a35458c70; v1 numbers preserved; secrets presence-only; prices/licenses live-sourced); degrade pipeline deterministic + not faking scores. Minors (no action req): phi4mm stray @register_model decorator (harmless — no models.yaml entry so can't enter a run); scripts_device_perf.py dev-only IndexError/handle-leak; degrade() fixed-skew = documented design.
   UI (B) = 1 Critical + 1 Important + 1 Minor. ✅ CRITICAL FIXED (commit 99cfecd): path traversal via unsanitized model/bench in /api/samples + /api/example (PoC'd arbitrary *.jsonl read outside results dir) → added _safe_seg guard + regression test, 104 pass. ⚠️ IMPORTANT = SPEC-GOVERNED, NOT a bug: /api/page-image not license-gated — but design spec lines 188-191 DELIBERATELY scope the gate to the explorer gallery (gating the per-example image would break omnidocbench review). → OWNER POLICY QUESTION, surfaced, NOT auto-changed. Minor: broad except masks gold-lookup bugs as 200 (low sev, read-only tool).
   ✅ OWNER DECIDED (2026-07-09, all 3 approved): (a) page-image stays UNGATED per spec (local review view), (b) MERGE approved, (c) UI polish = separate later round.
 (4) ✅ MERGED to main (merge commit 215d573, --no-ff; feature branch expansion/roster-reader-harness deleted; 104 tests pass on merged main; main is +31 ahead of origin — NOT pushed, owner to push). (5) ✅ CRONS DISARMED (3dd077d3 + b4e6461e both cancelled; CronList empty). (6) ✅ closing summary delivered.
 ⏸️ REMAINING (owner-triggered, NOT autonomous): UI polish pass on C3a dashboard; `git push origin main` when owner ready; optional page-image license-gating if policy changes. PLAN COMPLETE.
  ⚠️ SILENT FAILURE CAUGHT+FIXED (2026-07-09 hourly HC): first Part-D launch used `--profile full -b realdoc_qa_scanned_*` but the scanned caps live under a SEPARATE `partd_scanned` profile (matrix.yaml:35-40), NOT under full → `max_samples.get(scanned)`=None → UNCAPPED (running full ~1356/severity ≈ days). Caught when deepseek light hit 110>100. Killed by PID, trimmed deepseek overflow→100, RELAUNCHED with explicit `--max-samples 100` (scalar caps every bench at exactly samples[:100]; scanned emits same ids/order as clean → comparable). done_ids reuse deepseek's 100. Same cap-semantics trap as the earlier smoke — LESSON: for scanned/any bench not in the chosen profile's dict, ALWAYS pass scalar --max-samples.
  C3a DASHBOARD ⏳ subagent a9dcd31d122511aea (brief scratchpad/partC3_brief.md, report partC3_report.md): read-only `gauntlet ui` FastAPI + self-contained frontend (leaderboard/bench-explorer/per-example extracted-vs-truth). C3b (config UI + system-prompt) DEFERRED/owner-gated. Runs parallel to Part-D (reads results/runs, no write, no GPU — no conflict). On report: verify routes + confirm dead + server not orphaned.
  ✅ C3a DONE + VERIFIED (commits 0fa5bca spec, 98e6d8b impl; 101 tests). `gauntlet ui` (localhost-only) launches; independently verified /api/scoreboard (14 models), /api/example (real markdown+gold+score), /api/page-image (PNG), gallery license-gating; server killed clean, no orphan. C3b (interactive config UI + per-model system-prompt editor + write paths) DEFERRED — owner-gated (real UX decisions; flag to owner).
  PART-D CAP FIX CONFIRMED: relaunched run deepseek light=100 exactly (cap works), now on heavy. Correctly bounded.
  AFTER both: final whole-branch review + finishing-a-development-branch; DISARM crons 8e04eb47 + 67be2f84; final summary to owner.
...
CRONS (⚠️ SESSION-ONLY — die on restart; re-arm if a fresh session finds none via CronList while work ongoing): RE-ARMED 2026-07-09 ~17:15 after a session restart killed the originals — now **3dd077d3** (heartbeat :13,:43) + **b4e6461e** (hourly health :23). (dead originals were 8e04eb47/67be2f84.) Disarm both at completion.
PENDING (owner req 2026-07-09): UI POLISH PASS on the C3a dashboard — "looks nice but you can do better; focused pass later." Do AFTER Part D, as a dedicated design/UX pass (leaderboard+example strong; benches page slow under load). Not mid-run.
